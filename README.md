...
- 도메인: **`moodby.kr`** — repo 이름과 도메인은 별개
...
- **Cloudflare Pages 도메인**: `moodby.kr` 연결
- **CMS 인증**: `admin/config.yml` 의 `base_url`(공유 Sveltia Auth Worker) `ALLOWED_DOMAINS` 에 `moodby.kr` 추가
- **사진/영상 저장소(R2)**: `bucket: moodby-images`, `public_url: https://media.moodby.kr` 기준으로 버킷·미디어 서브도메인·전용 토큰 생성 (`access_key_id` 현재 자리표시자)
