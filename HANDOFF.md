...
## 0. 한 줄 요약
결혼식 사회자 **에이전시** 브랜드 **무드바이(MOOD BY)** 홈페이지. 순수 정적 사이트(HTML/CSS/JS) + Sveltia CMS 관리자 + Cloudflare Pages/R2. 코드는 거의 완성, **남은 건 주로 인프라 세팅(배포·도메인·R2·CMS 인증)**.
...
- **미디어**: 소량은 repo `images/`(예: 현재 히어로 `images/moodby-hero.jpg`), 대용량 원본은 Cloudflare R2(아직 미설정).
...
- `css/style.css` — 최상단 `:root` 변수로 색/폰트 제어. **메인 컬러: `--bg:#FFF8F2`, `--ink:#4E3D34`, `--accent:#C58A63`.** 신규 컴포넌트(.mc-card/.mc-kw/.pricing-table/.pipeline/.palette-row/.hero-word 등)는 파일 하단 "무드바이 신규 컴포넌트" 블록. 사회자별 색은 카드의 CSS 변수 `--mc-c`.
- `js/main.js` — 렌더러 전부. 핵심: 히어로 형용사 로테이터, MC 카드+무드 필터+상세 라이트박스(openMc), 가격표, 아카데미, 팔레트 점. `PALETTE` 폴백색 배열. **보통 수정 불필요.**
...
`brand`, `company`, `mcPrefix`("무드바이"), `instagram`, `kakao`(빈값),
...
## 6. 현재 상태 (완료)
- ✅ 무드바이 에이전시 구조로 전체 페이지 구현 + 컬러/팔레트 아이덴티티 반영, 로컬 렌더 검증(콘솔에러 0).
- ✅ 도메인 moodby.kr 코드 반영(CNAME/config/og).
- ✅ 히어로 이미지 추가됨(`images/moodby-hero.jpg`), MC strength/recommend 카피 채움.
- ⚠️ **이 변경분들이 아직 커밋/푸시 안 됐을 수 있음 → `git status` 확인 후 커밋·push 먼저.** 마지막 푸시 커밋: `760cae2`.

## 7. 남은 작업 — 인프라 (지금 시작 단계) ★ 핵심
moodby.kr 전용 값:
```
...
- 히어로 이미지는 `images/moodby-hero.jpg`로 들어가 있음(교체 가능).
